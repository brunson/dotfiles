...
(defconst py-version "$Revision: 4.1.1 $"
...
(if py-mode-map
  ...
  ;; doc keys
  ;;   NOTE: pydoc.el globably binds {C-c M-h} to (pydoc-commands)
  ;;   so don't reuse that key in this map.
  ;; electric keys
...
(and (py-safe (require 'easymenu) t)
     (easy-menu-define
      ...
      '("Python"
	["PyDoc Help Commands"  pydoc-commands t]
	"-"
...
py-beep-if-tab-change\t\tring the bell if `tab-width' is changed"
  ...
  (when (null py-which-shell)
    (py-toggle-shells py-default-interpreter t))
  )
...
This function is appropriate for `comint-output-filter-functions'."
  ...
  (when (and (or (string-equal string ">>> ")
    ...
    (let ((pyproc (get-buffer-process (current-buffer))))
      (goto-char (point-max))
      (move-marker (process-mark pyproc) (point))
...
(defun py-toggle-shells (arg &optional inhibit-message-p)
...
`jpython', equivalent to positive arg and negative arg respectively."
  ...
  (let (msg)
    ...
    (unless (or noninteractive inhibit-message-p)
...
(defun py-shell (&optional argprompt)
  "Start a Python interpreter in another window when called interactively.
When called programmatically, start the shell and make it the current buffer
temporarily (for the duration of the current command) but do not display it.
...
filter."
  ...
  ;; Set the default shell if not already set
  (when (null py-which-shell)
    (py-toggle-shells py-default-interpreter (not (interactive-p))))
  (let ((args py-which-args))
    ...
    ;; When called interactively, display the shell buffer.  When
    ;; called programmatically, use the shell as a background process 
    ;; for Python code evaluation and don't display it.
    (funcall (if (interactive-p) 'switch-to-buffer-other-window 'set-buffer)
...
Return the output buffer name or nil in some cases
...
is inserted at the end.  See also the command `py-clear-queue'."
  ...
  (let* ((proc (let ((proc (get-process py-which-bufname)))
		 ;; Use interactive Python buffer only if its
	...
	(write-region (point-min) (point-max) file nil 'nomsg))
      (setq output-buf (generate-new-buffer-name py-output-buffer))
      (let* (;; TBD: a horrible hack, but why create new Custom variables?
	     (arg (if (string-equal py-which-bufname "Python")
		      "-u" "")))
	(start-process py-which-bufname output-buf py-which-shell arg file)
	(pop-to-buffer output-buf)
	(py-postprocess-output-buffer output-buf)
	...
	(set-buffer input-buf)
	(write-region (point-min) (point-max) file nil 'nomsg))
      (setq output-buf (and (process-buffer proc)
	...
	(if (not (get-buffer py-output-buffer))
	    (message "No output.")
	  (setq output-buf py-output-buffer)
...
subtleties, including the use of the optional ASYNC argument."
  ...
  (if py-master-file
	...
	(set-buffer buffer)))
  (let ((py-execute-buffer t))
    ;; Above setting used by py-execute-region's subcall of
    ;; `py-jump-to-exception' to indicate that this is a full buffer
    ;; and not a region for which line numbers are unavailable.
...
subtleties, including the use of the optional ASYNC argument."
  ...
  (save-excursion
    ;; Reuse this buffer across calls; otherwise, too many buffers can
    ;; be created.
    (set-buffer (get-buffer-create " *Python Command*"))
    (setq buffer-read-only nil)
    (erase-buffer)
    ;; Must invoke python-mode here or the `py-which-shell' variable
    ;; will not be set and will trigger an error during the
    ;; `py-execute-region' call.
    (if (not (eq major-mode 'python-mode)) (python-mode))
    (insert string)
    (py-execute-region (point-min) (point-max) async)))
...
(defun py-jump-to-exception (file line)
  ...
  ;; Avoid jumping to the wrong line when a subregion from the current
  ;; buffer was sent for execution (since the line is improperly computed
  ;; from the temporary buffer holding the region).
  (unless (and (string-equal file "<stdin>")
	       (not (and (boundp 'py-execute-buffer) py-execute-buffer)))

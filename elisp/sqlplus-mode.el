...
(defconst sqlplus-revision "$Revision: 1.6 $")
...
(defun sqlplus (arg)
  ...
  (interactive "P")
  (if arg (kill-buffer sqlplus-buffer-name))
...
(defun sqlplus-restart nil
  "Kill an existing SQL*Plus buffer and restart"
  (interactive)
  (kill-buffer sqlplus-buffer-name)
  (sqlplus)
)
